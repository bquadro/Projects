...
JCSmartFilter.prototype.postHandler = function (result) {
		...
		if (!!result && !!result.ITEMS) {
				for (PID in result.ITEMS) {
                    ...
                    var id = 'range_cf_'+(arItem.PRICE?'P':'')+arItem.ID;
                    if(SFCurFilter && id == SFCurFilter) continue;

										var control = $('#'+arItem.VALUES.MIN.CONTROL_NAME);														
										var control_full = $('#'+arItem.VALUES.MIN.CONTROL_NAME+"_FULL");

                    var min = arItem.VALUES.MIN.HTML_VALUE ? arItem.VALUES.MIN.HTML_VALUE : arItem.VALUES.MIN.VALUE;
                    var max = arItem.VALUES.MAX.HTML_VALUE ? arItem.VALUES.MAX.HTML_VALUE : arItem.VALUES.MAX.VALUE;

            				var slider = control.parents('.kombox-range').find('.b_range_slide');
            				slider.data('range-from', arItem.VALUES.MIN.FILTERED_VALUE);
            				slider.data('range-to', arItem.VALUES.MAX.FILTERED_VALUE);
            			//	slider.ionRangeSlider("update");

                    slider.attr('data-value', min+';'+max);
                    ...
                    slider.ionRangeSlider("update", {                       
            				...
            				var slider = control_full.parents('.kombox-range').find('.b_range_slide');
            				slider.data('range-from', arItem.VALUES.MIN.FILTERED_VALUE);
            				slider.data('range-to', arItem.VALUES.MAX.FILTERED_VALUE);
            				//slider.ionRangeSlider("update");
