...
function init_filtr(filtr_name, type) {
		...
		$(filtr_name + " .b_box .b_title").click(function() {
				...
				var p = $(this).parent().find('.b_range .val_min');
        if(p.length) recalcRange(p, type);
...
//						$(this).parents(".b_range").find(".b_range_slide").slider({values: [min, max]})
				...
				var slider = $(this).find(".b_range_slide");
          ...
          onChange: function (obj) {
              slider.parent().find(".val_min").val(obj.fromNumber);
							slider.parent().find(".val_max").val(obj.toNumber);
          },
    			onFinish: function(obj){
      			    SFCurFilter = slider.attr("id");
								slider.parent().find(".val_min, .val_min_f").val(obj.fromNumber);
								slider.parent().find(".val_max, .val_max_f").val(obj.toNumber);
                ...
                filtr_dop.ionRangeSlider("update", {                       
          			...
          			filtr_dop.attr('data-value', obj.fromNumber+';'+obj.toNumber);
								filtr_dop.parent().find(".val_min, .val_min_f").val(obj.fromNumber);
								filtr_dop.parent().find(".val_max, .val_max_f").val(obj.toNumber);
                
                if (obj.fromNumber == min_v) {
										slider.parent().find(".val_min_f").val("");
										filtr_dop.parent().find(".val_min_f").val("");
								}
								if (obj.toNumber == max_v) {
										slider.parent().find(".val_max_f").val("");
										filtr_dop.parent().find(".val_max_f").val("");
								}

								smartFilter.reload(BX(slider.attr("id")));
								recalc_filtr_box();
    		  }
    		});
...
function recalcRange(_this, type) {
    ...
    p.find(".b_range_slide").ionRangeSlider("update", {                       
			from: min,                      
			to: max                     
		});
		
    p.find(".b_range_slide").attr('data-value', min+';'+max);
    ...
    filtr_dop.attr('data-value', min+';'+max);
